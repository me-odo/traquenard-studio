...
File size is a proxy, not the architecture rule. Review modules approaching the repository soft source budget for multiple reasons to change; crossing the hard production budget requires decomposition by durable author-facing/domain ownership or an explicit reviewed central exception. Never meet a budget by packing lines, hiding production logic in exclusions, adding ownership-free wrappers, or moving unrelated behavior into another large helper, registry, or `utils` module.

Shared behavior must not regrow a monolithic editor, Inspector, or registry. Keep semantic commands outside React presentation components. Before completing a substantial frontend change, inspect and report the largest changed handwritten production files. Add focused component/domain regression coverage and realistic desktop/mobile interaction evidence, then run the repository's targeted checks and full verification workflow.
