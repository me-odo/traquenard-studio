...
Implement the smallest coherent vertical change with boundary validation. Add focused unit/property/integration tests and a small E2E case when a user path changes. Update durable documentation only when the current contract changed; unfinished work belongs in a deduplicated GitHub Issue. Use targeted local tests during implementation, then run the complete local `pnpm verify`. Create coherent Conventional Commits only after it passes; normally make one push at the end and treat CI as independent confirmation, never as the debugging loop.
